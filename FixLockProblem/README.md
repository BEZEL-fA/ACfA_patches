# 概要
このフォルダには、ACfAのオンラインにおけるロックバグ修正パッチが含まれています。最新のパッチは以下の要素を修正します。
+ 右武器ロックバグの修正
...
注意点として、このパッチは"自分の画面の弾のみ"を修正します。正確に機能させるためには、双方のプレイヤーがパッチを持っている必要があります。
...
* imported_patch_old.yml
  * これは古いパッチファイルです。確立された、なおかつ安定した挙動を実現しています。しかし、片手のみの状態での命中率低下を防ぐことはできません。
...
* 0x9794ec
  ...
  * 関数は0x978aa0です。これはこのパッチのメイン関数です。
* 0x975084 / 0x97514c
  * 敵機体ポインタの読み込み関数です。読み込み元をリングバッファに変更することで安定したポインタ確保を行います。万一リングバッファが見つからない場合は、元々の処理を実行します。
  * 関数0x975038内で0x975084は実行されます。0x97514cは関数0x975100内で実行されます。メイン関数の0x979afcを経由してこの関数へジャンプします。
* 0x979b00
  * フラグ情報をリセットします。これもメイン関数で実行されます。
...
メインの制御関数は0x978aa0です。この関数が、全てのフックポイントを経由します。
